...
function getCards()
{
    global $db;
    $user_id = get_user_id();
    $q = $db->prepare('SELECT * FROM flashcards WHERE creator_id=:creator_id OR visibility=1');
...
function getCard($id = 0)
{
    ...
    global $db;
    $user_id = get_user_id();
    $q = $db->prepare('SELECT * FROM flashcards WHERE id=:id AND (creator_id=:creator_id OR visibility=1) LIMIT 1');
...
function getCardByLessonId($lesson_id = 0)
{
    ...
    global $db;
    $user_id = get_user_id();
    $q = $db->prepare('SELECT * FROM flashcards WHERE lesson_id=:lesson_id AND (creator_id=:creator_id OR visibility=1)');
...
function getCardsSinceLastUpdate()
{
    global $db;
    $user_id = get_user_id();
    $last_update_date = $_GET['last_update_date'];
    $q = $db->prepare('SELECT * FROM flashcards WHERE (creator_id=:creator_id OR visibility=1) AND modified > :last_update_date');
    $q->bindValue(':creator_id', $user_id, PDO::PARAM_INT);
